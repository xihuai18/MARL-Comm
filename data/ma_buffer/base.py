...
class MAReplayBuffer(ReplayBufferManager):
    ...
    def __init__(self,
        ...
        assert agent_num > 0
        assert not issubclass(buffer_cls,
                              ReplayBufferManager) or ma_env_num is not None

        buffer_list = [
            buffer_cls(total_size, ma_env_num, **kwargs) if issubclass(
                buffer_cls, ReplayBufferManager) else buffer_cls(
                    total_size, **kwargs) for _ in range(agent_num)
        ]
        super().__init__(buffer_list)

        self.buffer_num = sum([
            buf.buffer_num
            if issubclass(buffer_cls, ReplayBufferManager) else 1
    ...
    def get_agent_buffer(
            self, agent_id: int) -> Union[ReplayBuffer, ReplayBufferManager]:
